...
def extract_subjects(message:str,subjects_regex:str) -> set[str]:
    ...
    try:
        ...
        for subject in ALL_SUBJECTS:
            subject_match = re.findall(subject,raw_extracted_string,flags=re.IGNORECASE)
        ...
        return set(ALL_SUBJECTS) # Return a false positive
...
def extract_subject_levels(message:str,subject_levels_regex:str) -> set[str]:
    ...
    try:
        ...
        for level in ALL_SUBJECT_LEVELS:
            ...
            level_match = re.findall(level,raw_extracted_string,flags=re.IGNORECASE)
            if len(level_match) > 0: 
                subject_levels.add(level) # easy and compact implemmentation? wew...
        ...
        return set(ALL_SUBJECT_LEVELS) # Return a false positive
...
def extract_experience(message:str,experience_regex:str) -> set[str]:
    ...
    try:
        ...
        for experience_constant in ALL_TUTOR_EXPERIENCES_RANKED:
            found_experience = re.findall(experience_constant,raw_extracted_string,flags=re.IGNORECASE)
            if len(found_experience) > 0:
                print(f"Extracted experience from raw message: {found_experience}. Adding \"{experience_constant}\" to the list.")
                experience_required.add(experience_constant)
        ...
        if "above" in raw_extracted_string.lower():
            ...
            if len(experience_required) > 0:
                ...
                for ex in experience_required:
                    index = ALL_TUTOR_EXPERIENCES_RANKED.index(ex)
                    highest_level_index = min(highest_level_index, index)
                # Add in all levels above to the set.
                experience_required.update(ALL_TUTOR_EXPERIENCES_RANKED[highest_level_index:])
        ...
        if (len(experience_required) > 0): 
            ...
            experience_required = set(ALL_TUTOR_EXPERIENCES_RANKED) # just give back everything then
        return experience_required
    except Exception as e:
        print(f'⛔ Error occured while extracting experience: {e}. Error on line {e.__traceback__.tb_lineno}')
        return set(ALL_TUTOR_EXPERIENCES_RANKED) # Provide a false positive
...
def extract_gender_preference(message:str,gender_preference_regex:str) -> set[str]:
    ...
    try:
        ...
        if gender_preference_regex is not None:
            ...
            gender_match = re.findall("|".join(ALL_GENDERS),raw_extracted_string)
            if len(gender_match) > 0:
                for gender in ALL_GENDERS:
                    if gender in gender_match: gender_preference.add(gender)
        ...
        print(f"🚻 No preference indicated. Returning all genders: {set(ALL_GENDERS)}. Raw string: {raw_extracted_string}")
        return set(ALL_GENDERS)
    except Exception as e:
        print(f'⛔ Error occured while extracting gender preference: {e}. Error on line {e.__traceback__.tb_lineno}')
        return set(ALL_GENDERS) # Provide a false positive
